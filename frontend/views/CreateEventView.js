...
const { Option } = Select;
  ...
  const handleSubmit = (values) => {
    ...
    axios.post(`http://localhost:8080/api/events/create/${localStorage.getItem("userId")}`, eventData)
      .then(res => {
        console.log(res);
        eventId = res.data.id;
      })
  ...
  return (
    <Form layout="vertical" form={form} onFinish={handleSubmit} className="event-form">
    <Form.Item label="Event Title" name="eventTitle" rules={[{ required: true }]} className="form-item">
